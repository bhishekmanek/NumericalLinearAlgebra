...
!*************************************************************
! Gauss Jacobi iterative method for solving Ax=b
! Input : A,initial guess x,b,accuracy,msize
...
! Local Variables
real, allocatable :: err_vec(:),err_vec_print(:),y(:)
real :: error,summ,error1
integer :: i,j,k

open(1,file='Error_Iteration_J.dat',status='unknown')

allocate (err_vec(msize),y(msize),err_vec_print(msize))

! Iteration step k
k=0

! Initial error based on guess input vector x
do i=1,msize
...
k = k + 1
   ...
   do i=1,msize
      err_vec_print(i) = b(i) - dot_product(A(i,:),y(:))
   enddo

! Error calculation ||y-x|| and ||b-Ax||
   err_vec = y - x
   x = y
   error = euclid_norm(err_vec,msize)
   error1 = euclid_norm(err_vec_print,msize)
...
!***************************************************************
! Gauss-Seidel itearative method for solving Ax=b
! Input : A,b,accuracy,msize, initial guess x
! Output : final solution x
...
! Calculation of initial error based on input guess vector x
do i=1,msize
   err_vec(i) =  b(i) - dot_product(A(i,:),x(:))
enddo
...
do while (error > accuracy)
   ...
   do i=1,msize
      err_vec_print(i) = b(i)-dot_product(A(i,:),x(:))
   enddo
...
!************************************************************
! Cojugare gradient method to solve Ax=b
! Input - A,b,x,accuracy
! Output - x
...
! Initial error based on input guess x
do i=1,msize
   err_vec(i) =  b(i) - dot_product(A(i,:),x(:))
enddo
...
!**************************************************************
! Smart conjugate gradient method to solve Ax=b
! Input - A,b,x,accuracy
! Output - x
...
! Inital error based on guess input x
do i=1,msize
   err_vec(i) =  b(i) - dot_product(A(i,:),x(:))
enddo
...
! ********************************************************
! Diagnoal pre-conditioning applied to conjugate gradient method for solving Ax=b
! Input - A,b,x,accuracy
! Output - x

subroutine diag_pre_cond(A,b,msize,accuracy,x)
...
real, dimension(:), allocatable :: z,y,p,err_vec
real :: Enew,E,error,alpha,beta,tempe
integer :: i,j,k

allocate (z(msize),p(msize),err_vec(msize),y(msize))
...
p = z
E = dot_product(err_vec,z)
do while (sqrt(E) > accuracy)
   k = k + 1
   do i=1,msize
      y(i) = dot_product(A(i,:),p(:))
   enddo
   ...
   do i=1,msize
      z(i) = err_vec(i)/A(i,i)
   enddo
   Enew = dot_product(err_vec,z)
   beta = Enew/E
   p = z + beta*p
   E = Enew

write(*,*)'Error at Iteration step : ', k, 'is', sqrt(E)

enddo

end subroutine diag_pre_cond
