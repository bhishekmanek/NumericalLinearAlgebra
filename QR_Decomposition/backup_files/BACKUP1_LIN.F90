...
! 9. cholesky_fact - Performs Cholesky decomposition such that A=LL*
! 10. cholesky_back - Performs forward and back susbtitution on LL*x = b
...
!real, intent(OUT), allocatable :: s(:),v(:,:)
integer, intent(IN) :: msize, nsize

integer :: i,j,k
real :: summ,vec_norm
real, allocatable :: vec(:),tempv(:,:),mult(:,:),input(:),s(:),v(:,:)
real, allocatable :: iden(:,:),house_holde(:,:),temp_h(:,:)
real, allocatable :: check(:,:)

allocate (s(nsize),v(msize,nsize),vec(msize),tempv(nsize,msize),mult(msize,msize))
allocate (input(msize),temp_h(msize,msize))
allocate (iden(msize,msize),house_holde(msize,msize),q_mat(msize,msize))
allocate (check(msize,nsize))
...
do j=1,nsize
   ...
   input = matA(j:msize,j)
   summ = euclid_norm(input,msize-j+1)
   s(j) = sign(1.,matA(j,j))*summ
!   write(*,*)" The term -s(j) is ", -s(j)   
   v(j,j) = matA(j,j) + s(j)
   v(j+1:msize,j) = matA(j+1:msize,j)
   vec = v(j:msize,j)
  
! Vector v
   vec_norm =  euclid_norm(vec,msize-j+1)
   v(j:msize,j) = v(j:msize,j)/vec_norm

! Transpose of v
   tempv(j,j:msize) = v(j:msize,j)

! Calculating V*Vt
   mult(j:msize,j:msize) = matmul(v(j:msize,j:nsize),tempv(j:nsize,j:msize))
...
!   write(*,*)" H_jA at step ", j
   do i = 1,msize
!      write(*,*)(matA(i,k),k=1,nsize)
   enddo
...
!   do i = 1,msize
!      write(*,*)(check(i,j),j=1,nsize)
!   enddo

  deallocate (s,v,vec,tempv,mult,temp_h,input,iden)
...
! Input : Vector a and its length, Vector a is just the coefficients of fitting function
! Output : rms error
...
subroutine rms_error(a,msize,error)
