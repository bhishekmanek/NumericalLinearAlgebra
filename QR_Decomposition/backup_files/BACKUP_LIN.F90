...
subroutine qr_decomp(matA,msize,nsize,s,v)
...
integer :: i,j,k
real :: summ,vec_norm
real, allocatable :: vt(:,:),vec(:),temp(:,:),tempv(:,:),mult(:,:),val(:,:)

allocate (s(nsize),v(msize,nsize),vec(msize),temp(msize,nsize),tempv(1,msize),mult(4,4))
allocate (vt(msize,1),val(4,3))

do j=1,nsize
   summ = 0.0
   do i=j,msize    
      summ=summ+(matA(i,j)**2) 
      s(j) = sign(sqrt(summ),matA(j,j))
   enddo
   ...
   vec = v(1:msize,j)
   vec_norm =  euclid_norm(vec,msize)
   v(1:msize,j) = v(1:msize,j)/vec_norm
   tempv(1,1:msize) = v(1:msize,1)
   vt(:,1) = v(:,1)
   mult = matmul(vt,tempv)

enddo
!matA = matA - matmul(mult,matA)
val = matmul(mult,matA)
!do k=1,4,1
!tempv(1,k) = v(k,1)
!write(*,*)tempv(1,k),v(k,1)
!enddo
matA = matA - 2*val
do i=1,msize
